...
def train(epoch,loader,model,optimizer,writer):
    ...
    for i  in range(len(loader)):
         ...
         # adj = to_sparse(adj)
          ...
          if i%40 == 0:
             print("# {}/{} loss : {} , acc {}, time: {} , ETA:  {}".format(i,len_train,total_loss/count,total_acc/count,time.time()-t, (len_train-i)/40.0 * (time.time()-t) ))
             writer.add_scalar('train_loss', (total_loss.data)/count,(epoch*len_train + i)/40 )
             writer.add_scalar('train_acc', (total_acc.data)/count,(epoch*len_train + i)/40 )
             t = time.time()
          ...
          for j,x in enumerate(model.parameters()):
               ...
               if non_zero > 0 and non_zero < 100:   
                 writer.add_scalar(('non_zero_output_'),non_zero ,(epoch*len_train + i) )
    ...
    writer.add_scalar('epoch_train_loss', (total_loss)/count,(epoch) )
    writer.add_scalar('epoch_train_acc', (total_acc)/count,(epoch) )
...
loader = HopkinsDataset(window = 15 , root_dir = '../../NewHopkins/train_dataset_15/' )
print("len of dataset {}".format(len(loader)))
test_loader = HopkinsDataset(window = 15 , root_dir = '../../NewHopkins/val_dataset_15/' )
